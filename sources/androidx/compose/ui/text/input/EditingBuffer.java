...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m4220getMinimpl(j);
        this.selectionEnd = TextRange.m4219getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int iM4220getMinimpl = TextRange.m4220getMinimpl(j);
        int iM4219getMaximpl = TextRange.m4219getMaximpl(j);
        if (iM4220getMinimpl < 0 || iM4220getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + iM4220getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM4219getMaximpl < 0 || iM4219getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + iM4219getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM4220getMinimpl > iM4219getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + iM4220getMinimpl + " > " + iM4219getMaximpl);
        }
    }
    ...
    public final TextRange m4381getCompositionMzsxiRA$ui_text_release() {
        if (hasComposition$ui_text_release()) {
            return TextRange.m4210boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text_release, reason: not valid java name */
    public final long m4382getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long jM4383updateRangeAfterDeletepWDy79M = EditingBufferKt.m4383updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m4220getMinimpl(jM4383updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m4219getMaximpl(jM4383updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long jM4383updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m4383updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m4216getCollapsedimpl(jM4383updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m4220getMinimpl(jM4383updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m4219getMaximpl(jM4383updateRangeAfterDeletepWDy79M2);
            }
        }
    }
