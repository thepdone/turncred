...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m2330toArgb8_81llA(colors.get(i).m2286unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                fM2036getYimpl = Offset.m2035getXimpl(packedValue);
            } else {
                fM2036getYimpl = Offset.m2036getYimpl(packedValue);
            }
            fArr[i] = fM2036getYimpl;
        }
        return fArr;
    }
}
