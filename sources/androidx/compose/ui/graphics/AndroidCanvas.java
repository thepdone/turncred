...
public final class AndroidCanvas implements Canvas {
    ...
    public void mo2131concat58bKbWc(float[] matrix) {
        if (MatrixKt.m2532isIdentity58bKbWc(matrix)) {
            return;
        }
        android.graphics.Matrix matrix2 = new android.graphics.Matrix();
        AndroidMatrixConversions_androidKt.m2146setFromEL8BTi8(matrix2, matrix);
        this.internalCanvas.concat(matrix2);
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipRect-N_I0leg, reason: not valid java name */
    public void mo2130clipRectN_I0leg(float left, float top, float right, float bottom, int clipOp) {
        this.internalCanvas.clipRect(left, top, right, bottom, m2139toRegionOp7u2Bmg(clipOp));
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipPath-mtrdD-E, reason: not valid java name */
    public void mo2129clipPathmtrdDE(Path path, int clipOp) {
        android.graphics.Canvas canvas = this.internalCanvas;
        if (path instanceof AndroidPath) {
            canvas.clipPath(((AndroidPath) path).getInternalPath(), m2139toRegionOp7u2Bmg(clipOp));
            return;
        }
    ...
    public final Region.Op m2139toRegionOp7u2Bmg(int i) {
        return ClipOp.m2260equalsimpl0(i, ClipOp.INSTANCE.m2264getDifferencertfAjoo()) ? Region.Op.DIFFERENCE : Region.Op.INTERSECT;
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawLine-Wko1d7g, reason: not valid java name */
    public void mo2135drawLineWko1d7g(long p1, long p2, Paint paint) {
        this.internalCanvas.drawLine(Offset.m2035getXimpl(p1), Offset.m2036getYimpl(p1), Offset.m2035getXimpl(p2), Offset.m2036getYimpl(p2), paint.getInternalPaint());
    }
    ...
    public void mo2132drawCircle9KIMszo(long center, float radius, Paint paint) {
        this.internalCanvas.drawCircle(Offset.m2035getXimpl(center), Offset.m2036getYimpl(center), radius, paint.getInternalPaint());
    }
    ...
    public void mo2133drawImaged4ec7I(ImageBitmap image, long topLeftOffset, Paint paint) {
        this.internalCanvas.drawBitmap(AndroidImageBitmap_androidKt.asAndroidBitmap(image), Offset.m2035getXimpl(topLeftOffset), Offset.m2036getYimpl(topLeftOffset), paint.getInternalPaint());
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawImageRect-HPBpro0, reason: not valid java name */
    public void mo2134drawImageRectHPBpro0(ImageBitmap image, long srcOffset, long srcSize, long dstOffset, long dstSize, Paint paint) {
        ...
        rect.left = IntOffset.m4865getXimpl(srcOffset);
        rect.top = IntOffset.m4866getYimpl(srcOffset);
        rect.right = IntOffset.m4865getXimpl(srcOffset) + IntSize.m4907getWidthimpl(srcSize);
        rect.bottom = IntOffset.m4866getYimpl(srcOffset) + IntSize.m4906getHeightimpl(srcSize);
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        Intrinsics.checkNotNull(rect2);
        rect2.left = IntOffset.m4865getXimpl(dstOffset);
        rect2.top = IntOffset.m4866getYimpl(dstOffset);
        rect2.right = IntOffset.m4865getXimpl(dstOffset) + IntSize.m4907getWidthimpl(dstSize);
        rect2.bottom = IntOffset.m4866getYimpl(dstOffset) + IntSize.m4906getHeightimpl(dstSize);
    ...
    public void mo2136drawPointsO7TthRY(int pointMode, List<Offset> points, Paint paint) {
        if (PointMode.m2578equalsimpl0(pointMode, PointMode.INSTANCE.m2582getLinesr_lszbg())) {
            drawLines(points, paint, 2);
        } else if (PointMode.m2578equalsimpl0(pointMode, PointMode.INSTANCE.m2584getPolygonr_lszbg())) {
            drawLines(points, paint, 1);
        } else if (PointMode.m2578equalsimpl0(pointMode, PointMode.INSTANCE.m2583getPointsr_lszbg())) {
    ...
    private final void drawLines(List<Offset> points, Paint paint, int stepBy) {
        if (points.size() >= 2) {
            ...
            while (i < points.size() - 1) {
                ...
                this.internalCanvas.drawLine(Offset.m2035getXimpl(packedValue), Offset.m2036getYimpl(packedValue), Offset.m2035getXimpl(packedValue2), Offset.m2036getYimpl(packedValue2), paintAsFrameworkPaint);
                i += stepBy;
            }
        }
    }
    ...
    public void mo2137drawRawPointsO7TthRY(int pointMode, float[] points, Paint paint) {
    ...
    public void mo2138drawVerticesTPEHhCM(Vertices vertices, int blendMode, Paint paint) {
        this.internalCanvas.drawVertices(AndroidVertexMode_androidKt.m2182toAndroidVertexModeJOOmi9M(vertices.getVertexMode()), vertices.getPositions().length, vertices.getPositions(), 0, vertices.getTextureCoordinates(), 0, vertices.getColors(), 0, vertices.getIndices(), 0, vertices.getIndices().length, paint.getInternalPaint());
    }
    ...
    private final void drawPoints(List<Offset> points, Paint paint) {
        ...
        for (int i = 0; i < size; i++) {
            long packedValue = points.get(i).getPackedValue();
            this.internalCanvas.drawPoint(Offset.m2035getXimpl(packedValue), Offset.m2036getYimpl(packedValue), paint.getInternalPaint());
        }
    }
}
