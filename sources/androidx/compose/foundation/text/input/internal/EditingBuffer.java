...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m4222getStartimpl(j);
        this.selectionEnd = TextRange.m4217getEndimpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        checkRange(TextRange.m4222getStartimpl(j), TextRange.m4217getEndimpl(j));
    }
    ...
    public final void delete(int start, int end) {
        ...
        PartialGapBuffer.replace$default(this.gapBuffer, TextRange.m4220getMinimpl(jTextRange), TextRange.m4219getMaximpl(jTextRange), "", 0, 0, 24, null);
        ...
        setSelectionStart(TextRange.m4222getStartimpl(jM1465updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m4217getEndimpl(jM1465updateRangeAfterDeletepWDy79M));
        if (hasComposition()) {
            ...
            if (TextRange.m4216getCollapsedimpl(jM1465updateRangeAfterDeletepWDy79M2)) {
                commitComposition();
            } else {
                this.compositionStart = TextRange.m4220getMinimpl(jM1465updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m4219getMaximpl(jM1465updateRangeAfterDeletepWDy79M2);
            }
        }
        this.highlight = null;
    }
    ...
    public final void m1464setHighlightK7f2yys(int type, int start, int end) {
        if (start >= end) {
            throw new IllegalArgumentException("Do not set reversed or empty range: " + start + " > " + end);
        }
        this.highlight = new Pair<>(TextHighlightType.m1434boximpl(type), TextRange.m4210boximpl(TextRangeKt.TextRange(RangesKt.coerceIn(start, 0, getLength()), RangesKt.coerceIn(end, 0, getLength()))));
    }
