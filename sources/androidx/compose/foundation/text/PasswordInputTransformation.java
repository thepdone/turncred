...
public final class PasswordInputTransformation implements InputTransformation {
    ...
    public void transformInput(TextFieldBuffer textFieldBuffer) {
        if (textFieldBuffer.getChanges().getChangeCount() != 1 || TextRange.m4218getLengthimpl(textFieldBuffer.getChanges().mo1427getRangejx7JFs(0)) != 1 || TextRange.m4218getLengthimpl(textFieldBuffer.getChanges().mo1426getOriginalRangejx7JFs(0)) != 0 || textFieldBuffer.hasSelection()) {
            setRevealCodepointIndex(-1);
            return;
        }
        int iM4220getMinimpl = TextRange.m4220getMinimpl(textFieldBuffer.getChanges().mo1427getRangejx7JFs(0));
        if (getRevealCodepointIndex$foundation_release() != iM4220getMinimpl) {
            this.scheduleHide.invoke();
            setRevealCodepointIndex(iM4220getMinimpl);
        }
    }
