...
public final class TextDelegate {
    ...
    public /* synthetic */ TextDelegate(AnnotatedString annotatedString, TextStyle textStyle, int i, int i2, boolean z, int i3, Density density, FontFamily.Resolver resolver, List list, int i4, DefaultConstructorMarker defaultConstructorMarker) {
        this(annotatedString, textStyle, (i4 & 4) != 0 ? Integer.MAX_VALUE : i, (i4 & 8) != 0 ? 1 : i2, (i4 & 16) != 0 ? true : z, (i4 & 32) != 0 ? TextOverflow.INSTANCE.m4675getClipgIe3tQ8() : i3, density, resolver, (i4 & 256) != 0 ? CollectionsKt.emptyList() : list, null);
    }
    ...
    private final MultiParagraph m1374layoutTextK40F9xA(long constraints, LayoutDirection layoutDirection) {
        layoutIntrinsics(layoutDirection);
        int iM4692getMinWidthimpl = Constraints.m4692getMinWidthimpl(constraints);
        int iM4690getMaxWidthimpl = ((this.softWrap || TextOverflow.m4668equalsimpl0(this.overflow, TextOverflow.INSTANCE.m4676getEllipsisgIe3tQ8())) && Constraints.m4686getHasBoundedWidthimpl(constraints)) ? Constraints.m4690getMaxWidthimpl(constraints) : Integer.MAX_VALUE;
        int i = (this.softWrap || !TextOverflow.m4668equalsimpl0(this.overflow, TextOverflow.INSTANCE.m4676getEllipsisgIe3tQ8())) ? this.maxLines : 1;
        if (iM4692getMinWidthimpl != iM4690getMaxWidthimpl) {
            iM4690getMaxWidthimpl = RangesKt.coerceIn(getMaxIntrinsicWidth(), iM4692getMinWidthimpl, iM4690getMaxWidthimpl);
        }
        return new MultiParagraph(getNonNullIntrinsics(), Constraints.INSTANCE.m4699fitPrioritizingWidthZbe2FdA(0, iM4690getMaxWidthimpl, 0, Constraints.m4689getMaxHeightimpl(constraints)), i, TextOverflow.m4668equalsimpl0(this.overflow, TextOverflow.INSTANCE.m4676getEllipsisgIe3tQ8()), null);
    }
    ...
    public final TextLayoutResult m1376layoutNN6EwU(long constraints, LayoutDirection layoutDirection, TextLayoutResult prevResult) {
        if (prevResult != null && TextLayoutHelperKt.m1409canReuse7_7YC6M(prevResult, this.text, this.style, this.placeholders, this.maxLines, this.softWrap, this.overflow, this.density, layoutDirection, this.fontFamilyResolver, constraints)) {
            return prevResult.m4193copyO0kMr_c(new TextLayoutInput(prevResult.getLayoutInput().getText(), this.style, prevResult.getLayoutInput().getPlaceholders(), prevResult.getLayoutInput().getMaxLines(), prevResult.getLayoutInput().getSoftWrap(), prevResult.getLayoutInput().getOverflow(), prevResult.getLayoutInput().getDensity(), prevResult.getLayoutInput().getLayoutDirection(), prevResult.getLayoutInput().getFontFamilyResolver(), constraints, (DefaultConstructorMarker) null), ConstraintsKt.m4704constrain4WqzIAM(constraints, IntSizeKt.IntSize(TextDelegateKt.ceilToIntPx(prevResult.getMultiParagraph().getWidth()), TextDelegateKt.ceilToIntPx(prevResult.getMultiParagraph().getHeight()))));
        }
        MultiParagraph multiParagraphM1374layoutTextK40F9xA = m1374layoutTextK40F9xA(constraints, layoutDirection);
        return new TextLayoutResult(new TextLayoutInput(this.text, this.style, this.placeholders, this.maxLines, this.softWrap, this.overflow, this.density, layoutDirection, this.fontFamilyResolver, constraints, (DefaultConstructorMarker) null), multiParagraphM1374layoutTextK40F9xA, ConstraintsKt.m4704constrain4WqzIAM(constraints, IntSizeKt.IntSize(TextDelegateKt.ceilToIntPx(multiParagraphM1374layoutTextK40F9xA.getWidth()), TextDelegateKt.ceilToIntPx(multiParagraphM1374layoutTextK40F9xA.getHeight()))), null);
    }
