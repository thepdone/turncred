...
final class ParentSizeNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo412measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        int iM4692getMinWidthimpl = iRound != Integer.MAX_VALUE ? iRound : Constraints.m4692getMinWidthimpl(j);
        int iM4691getMinHeightimpl = iRound2 != Integer.MAX_VALUE ? iRound2 : Constraints.m4691getMinHeightimpl(j);
        if (iRound == Integer.MAX_VALUE) {
            iRound = Constraints.m4690getMaxWidthimpl(j);
        }
        if (iRound2 == Integer.MAX_VALUE) {
            iRound2 = Constraints.m4689getMaxHeightimpl(j);
        }
        final Placeable placeableMo3613measureBRTryo0 = measurable.mo3613measureBRTryo0(ConstraintsKt.Constraints(iM4692getMinWidthimpl, iRound, iM4691getMinHeightimpl, iRound2));
        return MeasureScope.layout$default(measureScope, placeableMo3613measureBRTryo0.getWidth(), placeableMo3613measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.lazy.ParentSizeNode$measure$1
            {
                super(1);
            }
