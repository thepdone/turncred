...
public final class LazyGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (layoutInfo.getOrientation() == Orientation.Vertical) {
            iM4865getXimpl = IntOffset.m4866getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            iM4865getXimpl = IntOffset.m4865getXimpl(lazyGridItemInfo2.getOffset());
        }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (iIntValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == iIntValue) {
                    if (z) {
                        iM4907getWidthimpl = IntSize.m4906getHeightimpl(visibleItemsInfo.get(i).getSize());
                    } else {
                        iM4907getWidthimpl = IntSize.m4907getWidthimpl(visibleItemsInfo.get(i).getSize());
                    }
                    iMax = Math.max(iMax, iM4907getWidthimpl);
                    i++;
                }
                i2 += iMax;
                i3++;
